...
void AssignExpr::FieldAssignment(){
	...
	VarDecl * var = dynamic_cast<VarDecl*>(Lookup( l->field,false));
	ClassDecl* cls = dynamic_cast<ClassDecl*>(var->GetParent());

	if(l->base == NULL && cls == NULL){//lvalue != member in class
		left->Emit();
		right->Emit();
		codegen->GenAssign(left->GetAddr(),right->GetAddr());
	}
	
	else if((l->base == NULL|| dynamic_cast<This*>(l->base))&& cls != NULL){
...
void RelationalExpr::Emit(){
	...
	if(strcmp(opName,">=")==0){
		...
		Location* tp2 = codegen->GenBinaryOp("==",right->GetAddr(),left->GetAddr());
...
void LogicalExpr::Emit(){
	...
	if(left){
		...
		Assert(strcmp(opName, "!")==0);
		Location * tp=codegen->GenLoadConstant(0);
		MemAddr = codegen->GenBinaryOp("==",right->GetAddr(),tp);
	}
...
void FieldAccess::Emit(){
	...
	VarDecl * var = dynamic_cast<VarDecl*>(Lookup(field,false));
	ClassDecl* cls = dynamic_cast<ClassDecl*>(var->GetParent());
	
	if(base == NULL && cls == NULL){//lvalue != member in class
	    //var = dynamic_cast<VarDecl*>(Lookup(field, false));
		MemAddr = var->GetAddr();
		Assert(MemAddr!=NULL);
	}

	else if((base == NULL|| dynamic_cast<This*>(base))&& cls != NULL){
		//refer lvalue = member in class
...
void NewArrayExpr::Emit(){
	...
	Location * tmp6 = codegen->GenBinaryOp("*", tmp4, tmp5);
