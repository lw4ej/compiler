...
template <class Value> void Hashtable<Value>::Insert( Hashtable<Value> * t)
{
	typename std::multimap<const char*, Value>::iterator itr;
	for(itr= t->getmmap()->begin(); itr!=t->getmmap()->end(); itr++){
		if(Lookup(itr->first)==NULL)
			Enter(itr->first,itr->second,false);
	}
}
/* Hashtable::Lookup
 * -----------------
