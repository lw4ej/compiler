...
template<class Value> class Hashtable {
  ...
  private: 
   ...
   public:
            // Actor creates a new empty hashtable
     Hashtable() {}

	 std::multimap<const char*, Value, ltstr> * getmmap(){return &mmap;}
     ...
     void Enter(const char *key, Value value,
		    bool overwriteInsteadOfShadow = false);
...
#include "hashtable.cc" // tricky, but allows implicit template instantiation
